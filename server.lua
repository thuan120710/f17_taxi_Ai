...
local getPlayerMoney = function(src)
    local totalMoney = 0
    if Config.Framework == 'ESX' then
        local ESX = GetESX()
...
local deductPlayerMoney = function(src, amount)
    if Config.Framework == 'ESX' then
        local ESX = GetESX()
        ...
        if tienkhoaMoney >= amount then
            xPlayer.removeAccountMoney('tienkhoa', amount)
        else
            if tienkhoaMoney > 0 then
                xPlayer.removeAccountMoney('tienkhoa', tienkhoaMoney)
                xPlayer.removeAccountMoney('bank', amount - tienkhoaMoney)
            else
    ...
    elseif Config.Framework == 'QBCore' then
        ...
        if tienkhoaMoney >= amount then
            Player.Functions.RemoveMoney('tienkhoa', amount)
        else
            if tienkhoaMoney > 0 then
                Player.Functions.RemoveMoney('tienkhoa', tienkhoaMoney)
                Player.Functions.RemoveMoney('bank', amount - tienkhoaMoney)
            else
...
RegisterNetEvent('msk_aitaxi:checkCallTaxi', function(basePrice)
    ...
    if getPlayerMoney(src) < basePrice then
